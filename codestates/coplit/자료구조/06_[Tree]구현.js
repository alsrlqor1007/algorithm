class Tree {
    ...
    insertNode(value) {
        ...
        const childNode = new Tree(value); // Tree 구조의 자식노드 생성
        this.children.push(childNode); // 생성한 자식노드를 루트노드의 자식으로 추가
    }

    // 트리 안에 해당 값이 포함되어 있는지 확인하는 메서드를 만듭니다.
    contains(value) {
        // TODO: 값이 포함되어 있다면 true를 반환하세요. 
        if (this.value === value) { // 해당 노드에 값이 바로 존재할 경우
            return true;
        }
        // TODO: 값을 찾을 때까지 children 배열을 순회하며 childNode를 탐색하세요.
        for (let i = 0; i < this.children.length; i++) { // 아래 tree 순회
            if (this.children[i].contains(value)) return true;
            // false일 경우 순회 종료되지 않게 조건문으로 수행
        }
