function createMatrix(edges) {
  let vertex = [];
  // 주어진 input에서 정점 값만 추출하기
  edges.forEach(el => vertex.push(el[0], el[1]));

  // 가장 큰 정점 N
  let max = Math.max(...vertex);
  // N에 맞게 0으로 채워진 빈 행렬 생성
  let matrix = new Array(max+1).fill(0).map(el => new Array(max+1).fill(0));

  // input 단방향, 양방향 조건 확인 후 간선 1 생성
  for (let el of edges) {
    let i = el[0];
    let j = el[1];

    if (el[2] === "directed") matrix[i][j] = 1;
    else {
      matrix[i][j] = 1;
      matrix[j][i] = 1;
    }
